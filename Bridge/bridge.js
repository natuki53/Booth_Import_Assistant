...
logDebug('IMPORTED_ASSETS_DIR:', IMPORTED_ASSETS_DIR);
...
try {
  ...
  if (!fs.existsSync(IMPORTED_ASSETS_DIR)) {
    logInfo('ImportedAssetsディレクトリを作成:', IMPORTED_ASSETS_DIR);
  ...
  try {
    fs.watch(DOWNLOADS_DIR, async (eventType, filename) => {
      ...
      if (boothMatch) {
        ...
        logInfo('✓ ZIP検知（命名規則）:', filename, '→ ID:', boothId);
        if (subFolder) {
          logInfo('  バリエーション:', subFolder);
        }
      } else {
        logDebug('命名規則にマッチしませんでした');
      }
      
      // パターン2: 任意のファイル名で、downloadTrackingMapに登録済み
      if (!boothId && downloadTrackingMap.has(filename)) {
        logDebug('パターン2: ダウンロード追跡マップから特定を試みます...');
        const tracking = downloadTrackingMap.get(filename);
        boothId = tracking.boothId;
        logInfo('✓ ZIP検知（追跡登録）:', filename, '→ ID:', boothId);
        logDebug('追跡情報:', tracking);
        
        // 追跡情報から削除（使い終わったら削除）
        downloadTrackingMap.delete(filename);
        logDebug('追跡情報を削除しました');
      } else if (!boothId) {
        logDebug('ダウンロード追跡マップにも登録がありませんでした');
        logDebug('現在の追跡マップサイズ:', downloadTrackingMap.size);
      }
      
      // 商品IDが特定できた場合のみ展開
      if (boothId) {
        logInfo('商品ID特定成功 - ZIP展開を開始します');
        await extractZip(zipPath, boothId, subFolder);
      } else {
        logWarn('⚠️ 商品IDを特定できませんでした');
        logWarn('ファイル名:', filename);
        logWarn('このファイルはBOOTHからのダウンロードではないか、');
        logWarn('ダウンロード前にBOOTHライブラリページで同期を行っていない可能性があります。');
        logInfo('手動対応方法: ファイルを "booth_<商品ID>.zip" にリネームしてください');
      }
